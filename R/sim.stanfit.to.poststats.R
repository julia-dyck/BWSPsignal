...
sim.stanfit.to.poststats = function(pc, stanfit.object, cred.niveaus = seq(0.5, 0.95, by = 0.05)){
  ...
  if(pc$tte.dist == "w"){
    ...
    if(pc$prior.dist == "fl" || pc$prior.dist == "fg"){
      post_summary = rstan::summary(obj, pars = c("theta", "nu"), probs = c())$summary
      poststats = list(th = post_summary["theta",], nu = post_summary["nu",], ga = post_summary["gamma",])
    }
    # else (ie. if the scale parameter is fix):
    if(pc$prior.dist == "ll" || pc$prior.dist == "gg"){
      post_summary = rstan::summary(obj, pars = c("nu", "gamma"), probs = c())$summary
      poststats = list(th = rep(NA, 5), nu = post_summary["nu",], ga = post_summary["gamma",])
    }
